...
def main():
    rospy.init_node('ego_vehicle_status_publisher', anonymous=False)
    pub = rospy.Publisher('/Competition_topic', EgoVehicleStatusMsg, queue_size=10)

    egovehiclestatus = Receiver(IP, PORT, EgoVehicleStatus())

    rospy.loginfo("EgoVehicleStatus publisher started on port %d" % PORT)
    ...
    while not rospy.is_shutdown():
        ...
        pub.publish(msg)
        rospy.loginfo_throttle(2.0, "Published EgoVehicleStatus to /Competition_topic")
        rate.sleep()
